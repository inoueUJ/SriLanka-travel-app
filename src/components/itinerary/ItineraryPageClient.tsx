...
export default function ItineraryPageClient() {
  ...
  const [progressData, setProgressData] = useState<Record<string, number>>({}); // 進捗データ用の状態を追加
      ...
      if (savedData) {
        ...
        Object.keys(itineraryData).forEach(dayId => {
          if (parsedData[dayId]) {
            ...
            // 進捗データを更新
            setProgressData(prev => ({...prev, [dayId]: dayProgress}));
          }
        });
      } else {
  ...
  const handleToggleItem = (itemId: string) => {
    ...
    // 現在のデイの進捗を更新
    const totalItems = updatedDayData.items.length;
    const completedItems = updatedDayData.items.filter(item => item.completed).length;
